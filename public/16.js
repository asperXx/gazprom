(window["webpackJsonp"] = window["webpackJsonp"] || []).push([[16],{

/***/ "./node_modules/vue-loader/lib/loaders/templateLoader.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/faq.vue?vue&type=template&id=7224101b&":
/*!******************************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!./node_modules/vue-loader/lib??vue-loader-options!./resources/js/components/faq.vue?vue&type=template&id=7224101b& ***!
  \******************************************************************************************************************************************************************************************************/
/*! exports provided: render, staticRenderFns */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "render", function() { return render; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "staticRenderFns", function() { return staticRenderFns; });
var render = function() {
  var _vm = this
  var _h = _vm.$createElement
  var _c = _vm._self._c || _h
  return _c(
    "v-card",
    { staticClass: "pa-5 ma-2" },
    [
      _c("h2", [_vm._v("FAQ")]),
      _vm._v(" "),
      _c(
        "v-expansion-panels",
        [
          _c(
            "v-expansion-panel",
            [
              _c("v-expansion-panel-header", [_vm._v("Что такое огоньки?")]),
              _vm._v(" "),
              _c("v-expansion-panel-content", [
                _vm._v(
                  "\n        Огоньки - внутренняя валюта портала, используемая для поощрения наиболее инициативных сотрудников компании.\n      "
                )
              ])
            ],
            1
          ),
          _vm._v(" "),
          _c(
            "v-expansion-panel",
            [
              _c("v-expansion-panel-header", [
                _vm._v("Для чего нужны огоньки и как их получить?")
              ]),
              _vm._v(" "),
              _c("v-expansion-panel-content", [
                _c("ol", [
                  _c("li", [
                    _vm._v(
                      "Каждому сотруднику за вход в систему выдаётся по 1 огоньку 1 раз в день."
                    )
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v(
                      "Валюта может быть использована сотрудниками в качестве комплемента своим коллегам. Сотрудник выбирает качество коллеги, характеризующее его работу (компетентность, надежность, динамичность, командность, ответственность), и указывает количество огоньков для передачи."
                    )
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v(
                      "При внесении статьи в пункт «Предложения», сотруднику начисляются огоньки, в зависимости от типа статьи:\n            решение проблемы - 50 огоньков, в случае, если же идея принимается ответственным подразделением (решение остается на сайте, но голосование закрывается) сотрудник получает общую сумму огоньков, равную количеству огоньков, которым проголосовали за его статью (блок предложения выделяется синим градиентом в зависимости от количества набранных огоньков).\n            проблема – 25 огоньков, в случае же, если проблема признана сотрудниками актуальной (блок проблемы выделяется красным градиентом при 100 набранных огоньков), то огоньки набранные статьей и последующие начисляются автору.\n          "
                    )
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v(
                      "При написании статьи в пункте «Делимся опытом», пользователь получает фиксированную сумму – 50 огоньков + лайки за статью, конвертированные в огоньки в соотношении 1:1."
                    )
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v(
                      "Огоньки можно тратить во внутреннем магазине, где представлены различные мерчи, купоны и сертификаты от партнеров."
                    )
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v(
                      "В случае, когда пользователь проголосовал огоньками за решение проблемы, которое было принято ответственным подразделением к исполнению – пользователь получает удвоенное количество огоньков, инвестированных в статью."
                    )
                  ])
                ])
              ])
            ],
            1
          ),
          _vm._v(" "),
          _c(
            "v-expansion-panel",
            [
              _c("v-expansion-panel-header", [
                _vm._v("Какие виды бейджей существуют?")
              ]),
              _vm._v(" "),
              _c("v-expansion-panel-content", [
                _vm._v(
                  '\n        В личном профиле реализуется "наградная доска", где размещаются добытые сотрудником бэйджи. Бэйджи делятся на следующие уровни:\n        '
                ),
                _c("ul", [
                  _c("li", [
                    _vm._v(
                      "значок – показатель активности сотрудника на портале."
                    )
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v(
                      "медаль – показатель особой активности сотрудника на портале."
                    )
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v(
                      "орден – высшая награда, выдаваемая за выявление чрезвычайно важных проблем или предложение решения принесшее небывалую прибыль."
                    )
                  ])
                ]),
                _vm._v("\n        Особенности:\n        "),
                _c("ul", [
                  _c("li", [
                    _vm._v(
                      "По каждому из бэйджей составляется рейтинг сотрудников. Каждый сотрудник может посмотреть свой рейтинг по данному бэйджу среди других сотрудников."
                    )
                  ]),
                  _vm._v(" "),
                  _c("li", [_vm._v("Все бэйджи имеют дату присвоения. ")]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v(
                      "Значки накапливаются и по накоплению определённого количества значков выдаётся соответствующая медаль."
                    )
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v(
                      "Ордена кроме даты присвоения имеют поля описания, в котором содержится информация за что получен орден. "
                    )
                  ])
                ])
              ])
            ],
            1
          ),
          _vm._v(" "),
          _c(
            "v-expansion-panel",
            [
              _c("v-expansion-panel-header", [
                _vm._v("Какие есть значки, медали и ордена?")
              ]),
              _vm._v(" "),
              _c("v-expansion-panel-content", [
                _vm._v("\n        Значки:\n        "),
                _c("ul", [
                  _c("li", [
                    _vm._v("Передовик – автору 2 принятых актуальных решений. ")
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v("Исследователь – автору 4 актуальных проблем. ")
                  ]),
                  _vm._v(" "),
                  _c("li", [_vm._v("Писатель – за 3 опубликованные статьи. ")]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v(
                      "Лидер офиса, Лидер филиала, Лидер компании – присваивается за лидерство среди сотрудников по написанным статьям."
                    )
                  ])
                ]),
                _vm._v("\n\n        Медали:\n        "),
                _c("ul", [
                  _c("li", [
                    _vm._v("Локомотив -  5 раз получен значок «Передовик». ")
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v("Эйнштейн – 3 раза получен значок «Исследователь».")
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v("Издатель – 10 раз получен значок «Писатель». ")
                  ])
                ]),
                _vm._v("\n\n        Ордена:\n        "),
                _c("ul", [
                  _c("li", [
                    _vm._v(
                      "Спасатель - за выявление чрезвычайно важной проблемы банка (отдела)."
                    )
                  ]),
                  _vm._v(" "),
                  _c("li", [
                    _vm._v(
                      "Оптимизатор - за реализованную идею, повлекшую рост показателей банка (отдела)."
                    )
                  ])
                ])
              ])
            ],
            1
          ),
          _vm._v(" "),
          _c(
            "v-expansion-panel",
            [
              _c("v-expansion-panel-header", [
                _vm._v("Присутствует ли элемент соревновательности?")
              ]),
              _vm._v(" "),
              _c("v-expansion-panel-content", [
                _vm._v(
                  "\n        Для каждого отдела реализуется своя доска лидеров по огонькам. При нахождении сотрудника на доске лидеров от 2 месяцев и более, он перемещается на доску лидеров региона (при этом сравниваются количества огоньков сотрудников разных отделений банка в одном регионе). Также существует доска лидеров всего банка, куда попадает региональный лидер после нахождения на доске региональных лидеров от 2 месяцев и более.\n      "
                )
              ])
            ],
            1
          ),
          _vm._v(" "),
          _c(
            "v-expansion-panel",
            [
              _c("v-expansion-panel-header", [
                _vm._v(
                  "Существует ли поощрение для самого инициативного отдела, филиала?"
                )
              ]),
              _vm._v(" "),
              _c("v-expansion-panel-content", [
                _vm._v(
                  "\n        Да, существует переходящий флаг. Под флагом понимается внесение изменений в дизайн портала для конкретного отдела (не координально, добавляются элементы «премиальности» в оформление, например рамка для фото у каждого сотрудника отдела и т.п.). Таковое совершается ежеквартально после сравнения показателей активности на портале между всеми одинаковыми по функционалу отделами банка. Общая активность оценивается по совокупному количеству добытых огоньков. \n      "
                )
              ])
            ],
            1
          )
        ],
        1
      )
    ],
    1
  )
}
var staticRenderFns = []
render._withStripped = true



/***/ }),

/***/ "./resources/js/components/faq.vue":
/*!*****************************************!*\
  !*** ./resources/js/components/faq.vue ***!
  \*****************************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _faq_vue_vue_type_template_id_7224101b___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./faq.vue?vue&type=template&id=7224101b& */ "./resources/js/components/faq.vue?vue&type=template&id=7224101b&");
/* harmony import */ var _node_modules_vue_loader_lib_runtime_componentNormalizer_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../../../node_modules/vue-loader/lib/runtime/componentNormalizer.js */ "./node_modules/vue-loader/lib/runtime/componentNormalizer.js");

var script = {}


/* normalize component */

var component = Object(_node_modules_vue_loader_lib_runtime_componentNormalizer_js__WEBPACK_IMPORTED_MODULE_1__["default"])(
  script,
  _faq_vue_vue_type_template_id_7224101b___WEBPACK_IMPORTED_MODULE_0__["render"],
  _faq_vue_vue_type_template_id_7224101b___WEBPACK_IMPORTED_MODULE_0__["staticRenderFns"],
  false,
  null,
  null,
  null
  
)

/* hot reload */
if (false) { var api; }
component.options.__file = "resources/js/components/faq.vue"
/* harmony default export */ __webpack_exports__["default"] = (component.exports);

/***/ }),

/***/ "./resources/js/components/faq.vue?vue&type=template&id=7224101b&":
/*!************************************************************************!*\
  !*** ./resources/js/components/faq.vue?vue&type=template&id=7224101b& ***!
  \************************************************************************/
/*! exports provided: render, staticRenderFns */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_faq_vue_vue_type_template_id_7224101b___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! -!../../../node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!../../../node_modules/vue-loader/lib??vue-loader-options!./faq.vue?vue&type=template&id=7224101b& */ "./node_modules/vue-loader/lib/loaders/templateLoader.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/faq.vue?vue&type=template&id=7224101b&");
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "render", function() { return _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_faq_vue_vue_type_template_id_7224101b___WEBPACK_IMPORTED_MODULE_0__["render"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "staticRenderFns", function() { return _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_faq_vue_vue_type_template_id_7224101b___WEBPACK_IMPORTED_MODULE_0__["staticRenderFns"]; });



/***/ })

}]);